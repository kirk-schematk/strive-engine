...
| `preview-home.html` | Home page mockup and source of truth for the hero **assessment card** (the self-contained `<style>` + `.qz` markup + `<script>` block, pasted into a Webflow HTML Embed on `/`). Three static taps (discipline → goal → warm-up), no Xano calls; **Continue** links to `/get-started?discipline=…&goal=…`, which `strive-onboard.js` reads to skip its choosers and start the assessment. |
