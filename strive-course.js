...
(function (global) {
  ...
  function renderPath(c) {
    ...
    var steps = c.modules.map(function (m, i) {
      var n = ('0' + (i + 1)).slice(-2);
      return '<div class="sc-step"><div class="sc-n">' + n + '</div>' +
        '<div class="sc-nm">' + esc(shortTitle(m.title)) + '</div>' +
        '<div class="sc-t">' + esc(m.time || '') + '</div></div>';
    }).join('');
  ...
  function load(opts) {
    ...
    if (opts.data) return Promise.resolve(render(opts.data, mount));
  ...
  global.STRIVECourse = { render: render, load: load, version: '1.0.0' };
